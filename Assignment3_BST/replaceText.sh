...
do
  ...
  sed -i "s/import\sjava.util.LinkedList;//g" $filename
  sed -i "s/import\sjava.util.List;//g" $filename
  sed -i "s/import\sjava.util.Arrays;//g" $filename
  sed -i "s/System.out.println(bstHeight(bstRoot));//g" $filename
done
